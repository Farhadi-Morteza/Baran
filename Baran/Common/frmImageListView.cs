...
{
    public partial class frmImageListView : Baran.Base_Forms.frmSecondChildBase
    {
        BaranDataAccess.Common.dstCommon.spr_cmn_DocumentByFkID_SelectDataTable tblDocument =
            new BaranDataAccess.Common.dstCommon.spr_cmn_DocumentByFkID_SelectDataTable();
        BaranDataAccess.Common.dstCommon.spr_cmn_DocumentByFkID_SelectRow rwDocument;
        ...
        #region Constructor
        public frmImageListView(BaranDataAccess.Common.dstCommon.spr_cmn_DocumentByFkID_SelectDataTable tblDoc)
        {
            InitializeComponent();
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            //int Counter = 1;
            foreach (var Doc in tblDocument)
            {
                //saveFileDialog1.FileName = Application.StartupPath + @"\Doc\" + Doc.Name + Counter.ToString() + ".Jpeg";
                //Counter++;
                saveFileDialog1.FileName = Application.StartupPath + @"\Doc\" + Doc.Name + Doc.DocumentID.ToString() + ".jpeg";
                System.IO.MemoryStream ms = new System.IO.MemoryStream(Doc.Document);
            ...
            var filesToCopy = Directory.EnumerateFiles(Application.StartupPath + @"\Doc\", "*.jpeg");
            int jj = Directory.GetFiles(PublicVariables.DocumentPath, "*", SearchOption.TopDirectoryOnly).Length;
            string[] filesnames = new string[jj];
            ...
            foreach (var file in filesToCopy)
            {
                ...
                //imageListView1.Items.AddRange(file);
            }

            Application.Idle += new EventHandler(Application_Idle);
            imageListView1.Items.AddRange(filesnames);


            for (int it = 0; it <= imageListView1.Items.Count - 1; it++)
            {
                imageListView1.Items[it].ForeColor = BaranLibrary.GeneralProperties.BaseControlForeColor;
            }
        }
        #endregion
        ...
        private void frmImageListView_Paint(object sender, PaintEventArgs e)
        {
        ...
        public override void OnCancel()
        {
            ...
            ////to delete  
            //FileInfo info = new FileInfo(Application.StartupPath + @"\Doc\");
            //if (info.Exists)
            //{
            //    info.Delete();
            //}

            //FileStream fs = File.OpenRead(Application.StartupPath + @"\Doc\");
            //System.IO.File.Delete(Application.StartupPath + @"\Doc\");
        }
        ...
        private void SaveAlltoolStripButton_Click(object sender, EventArgs e)
        {
            ...
            {
                var filesToCopy = Directory.EnumerateFiles(PublicVariables.DocumentPath);
                var directoryToCopyTo = fbd.SelectedPath;

                try
                {
                    foreach (var file in filesToCopy)
                    {
                        string SaveFile = directoryToCopyTo + "\\" + Path.GetFileName(file);
                        if (File.Exists(SaveFile))
                        {

                            DialogResult msgResult = MessageBoxX.ShowMessageBox(PublicEnum.EnmMessageType.msgFileExists, Path.GetFileName(file).ToString());
                            if (msgResult == DialogResult.Yes)
                            {
                                File.Copy(file, Path.Combine(directoryToCopyTo, Path.GetFileName(file)), true);
                            }
                            else
                            {
        ...
        private void SaveSelectedItemstoolStripButton_Click(object sender, EventArgs e)
        {
            if (imageListView1.SelectedItems.Count == 0)
            {
                OnMessage(BaranResources.NoItemsSelected, PublicEnum.EnmMessageCategory.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (fbd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (ImageListViewItem item in imageListView1.SelectedItems)
                    {
                        string SaveFile = directoryToCopyTo + "\\" + Path.GetFileName( item.FileName);
                        if (File.Exists(SaveFile))
                        {
                            DialogResult msgResult = MessageBoxX.ShowMessageBox(PublicEnum.EnmMessageType.msgFileExists, Path.GetFileName(item.FileName));
                            if (msgResult == DialogResult.Yes)
                            {
                                File.Copy(item.FileName, Path.Combine(directoryToCopyTo, Path.GetFileName(item.FileName)), true);
                            }
                            else
                            {
                        ...
                        {
                            var filesToCopy = item.FileName;
                            string sssss = Path.GetDirectoryName(filesToCopy);//.GetFileName(filesToCopy);
                            File.Copy(filesToCopy, Path.Combine(directoryToCopyTo, Path.GetFileName(filesToCopy)));
                        }
                    }
                    OnMessage(BaranResources.SaveSuccessful, PublicEnum.EnmMessageCategory.Success);
                }
